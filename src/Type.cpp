...
void Type::match(std::shared_ptr<DetectedObject> object) {
    ...
    if (obj_insts.empty()){
        ...
        // Count the occurrences of each image in the type.
        std::unordered_map<int,int> match_occurrences;
        /*
         * TODO: Replace this with weighting scheme (TF-IDF?) so that common features don't throw off the match.
         */
        for(int i = 0; i < match_imgs.size(); ++i){
            if (match_occurrences.count(match_imgs[i]) == 0){
                //std::cout << "First occurrence" << std::endl;
                match_occurrences[match_imgs[i]] = 1;
            }else {
                //std::cout << "Increment occurrence" << std::endl;
                match_occurrences[match_imgs[i]] += 1;
            }
        }
        //std::cout << match_occurrences[match_imgs[0]] << std::endl;
        int best_img_index = -1;
        int best_val = 0;
        // Get the best image index.
        for (std::unordered_map<int,int>::const_iterator it = match_occurrences.begin(); it != match_occurrences.end(); ++it){
            //std::cout << it->second << std::endl;
            if (it->second > best_val){
                best_img_index = it->first;
                best_val = it->second;
            }
        }
        ...
        // TODO: Find out why findHomography is core dumping (Different data type than before?)
        ...
        if (cv::countNonZero(mask) > INLIERS_THRESH){
            ...
            ROS_INFO_STREAM("Match found with object of type " << ClassMap::get_class(id) << " with ID: " << best_img_ptr->parent->get_id());
            ...
            ROS_INFO_STREAM("Not enough inliers for matched image " << best_img_index << " expected: " << INLIERS_THRESH << " got: " << cv::countNonZero(mask));
            add_new_obj(object);
        }
